...
from timm.models import named_apply, checkpoint_seq, adapt_input_conv
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, get_cur_feat: bool = False) -> torch.Tensor:
        B, N, C = x.shape
        if get_cur_feat:
            qkv = self.qkv(x, get_cur_feat=get_cur_feat).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        else:
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)  # [B, H, N, d] * 3
        q, k = self.q_norm(q), self.k_norm(k)
        if hasattr(self, (_reader := 'interm_reader_1')):
            q = getattr(self, _reader)(q)
        ...
        if self.fused_attn:
            ...
            if hasattr(self, (_reader := 'interm_reader_2')):
                attn = getattr(self, _reader)(attn)
...
class Prompt(nn.Module):
    def __init__(self, prompt_len: int, embed_dim: int = 768, prompt_init: str = 'uniform', replace_token: bool = False):
        super().__init__()
        assert prompt_len >= 0
    ...
    @property
    def pre_tokens(self):
        return self.__pre_tokens

    def forward(self, x: Tensor):
        batch_prompt = self.prompt.expand(x.shape[0], -1, -1)

        if self.replace_token:
            out = torch.concat([x[:, :self.pre_tokens], batch_prompt], dim=1)
            assert out.shape[1] == x.shape[1]
        else:
            out = torch.concat([x, batch_prompt], dim=1)
            assert out.shape[1] == x.shape[1] + self.prompt.shape[1]

        return out

    def extra_repr(self) -> str:
        return f"prompt_len={self.prompt_len}, embed_dim={self.embed_dim}, prompt_init={self.prompt_init}, replace_token={self.replace_token}"
...
class Block(nn.Module):
    def __init__(
        ...
        if prompt is not None:
            assert isinstance(prompt, Prompt), type(prompt)
            self.prompt = prompt
            pid_dict = {'pid1': id(self.prompt.prompt),
                        'pid2': id(self.prompt.prompt)}
        else:
            pid_dict = {}
    ...
    def forward(self, x: torch.Tensor, get_cur_feat: bool = False) -> torch.Tensor:
        if hasattr(self, 'prompt'):
            _fn1, _fn2 = self.prompt, self.norm1
            x_a = self.attn(_fn2(_fn1(x)), get_cur_feat=get_cur_feat)[:, :self.prompt.pre_tokens]
        else:
            x_a = self.attn(self.norm1(x),get_cur_feat=get_cur_feat)

        x = x + self.drop_path1(self.ls1(x_a))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            prompt_args_dict: dict[str, Any] = {},
            other_args_dict: dict[str, Any] = {},
    ) -> None:
        """
        Args:
        ...
        if len(_pd) == 0:
            ...
            self.blocks = nn.Sequential(*[
                block_fn(
                    ...
                    act_layer=act_layer,
                    mlp_layer=mlp_layer,
                    prompt=Prompt(_pd['prompt_len'], embed_dim, _pd['prompt_init'], False) if _pd['prompt_start_block'] <= i <= _pd['prompt_end_block'] and _pd['prompt_len'] > 0 else None,
                )
                for i in range(depth)])
        ...
        _logit_scale: float = other_args_dict['logit_scale']
        if other_args_dict['logit_scale_trainable']:
            self.logit_scale = nn.Parameter(torch.tensor(_logit_scale))
        else:
            self.register_buffer('logit_scale', torch.tensor(_logit_scale), persistent=False)

        self.logit_type: Literal['head_out', 'sim_imgtext'] = other_args_dict['logit_type']
    ...
    def forward_features(self, x: torch.Tensor, get_cur_feat: bool = False) -> torch.Tensor:
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if get_cur_feat:
                for i in range(len(self.blocks)):
                    x = self.blocks[i](x, get_cur_feat=get_cur_feat)
            else:
                x = self.blocks(x)
        x = self.norm(x)
        return x
    ...
    def cache_text_features(self, text_features: Tensor):
        _p = next(self.parameters())
        self.register_buffer('text_features', text_features.to(device=_p.device, dtype=_p.dtype), persistent=False)

    def remove_text_features(self):
        self.register_buffer('text_features', None, persistent=False)

    def encode_image(self, x, pre_logits: bool = False, get_cur_feat: bool = False):
        x = self.forward_features(x, get_cur_feat=get_cur_feat)
        x = self.forward_head(x, pre_logits)
        return x

    def forward_logits(self, image_features: Tensor):
        image_features = image_features / image_features.norm(dim=1, keepdim=True)
        logits_per_image = image_features @ self.get_buffer("text_features").t() * self.logit_scale.exp()
        return logits_per_image

    def forward(self, x: torch.Tensor, get_cur_feat: bool = False) -> torch.Tensor:
        x = self.encode_image(x, get_cur_feat=get_cur_feat)
        if self.logit_type == 'head_out':
            x = x
        elif self.logit_type == 'sim_imgtext':
            x = self.forward_logits(x)
        else:
            raise ValueError(self.logit_type)
        return x
...
register_model_deprecations(__name__, {
    ...
    'vit_base_patch16_224_in21k_ft_in1k':'vit_base_patch16_224.augreg2_in21k_ft_in1k',
})
