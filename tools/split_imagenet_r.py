...
root_dir = f"/root/PAPER/VPTinNSforCL/data/imagenet-r"

def split_func(mode):
    mode_dir = osp.join(root_dir, mode)
    os.makedirs(mode_dir, exist_ok=True)
    with open(f"/root/PAPER/VPTinNSforCL/tools/imagenet_r_{mode}.txt") as f:
        lines = f.readlines()
    for line in lines:
        rel_path = line.strip('\n')
        ...
        class_dir = osp.realpath(osp.join(mode_dir, osp.dirname(rel_path)))
