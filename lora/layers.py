...
class LoRALayer():
    ...
    # def merge_BA(self, param_name: str, x):
    #     lora_name = self.params_with_lora[param_name]
    #     # import pdb;pdb.set_trace()
    #     x_A = torch.matmul(x, eval(f'self.{lora_name}_lora_A').T)
    #     # if True:
    #     #     x_A = torch.relu(x_A)
    #     x_B = torch.matmul(x_A, eval(f'self.{lora_name}_lora_B').T)
    #     return x_B
...
class LinearLoRA(nn.Linear, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        if not isinstance(self.old_weight_1, int):
            self.old_weight_1 = self.old_weight_1.to(x.device)
        ...
        if self.r > 0 and not self.merged:
            ...
            # lora_adjustment = self.merge_BA('weight') * self.scaling 
...
class PlainMultiheadAttentionLoRA(nn.Module):
    def __init__(
        ...
        self.embed_dim = existing_mha.qkv.weight.shape[1]
